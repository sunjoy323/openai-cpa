...
def _ensure_db_manager_fallbacks() -> None:
    fallback_funcs = {
        "check_account_exists": lambda *_args, **_kwargs: False,
        "get_mailbox_for_pool_fission": lambda *_args, **_kwargs: None,
        "update_local_mailbox_status": lambda *_args, **_kwargs: None,
        "update_local_mailbox_refresh_token": lambda *_args, **_kwargs: None,
        "get_and_lock_unused_local_mailbox": lambda *_args, **_kwargs: None,
        "clear_retry_master_status": lambda *_args, **_kwargs: None,
    }
    for name, func in fallback_funcs.items():
        if not hasattr(db_manager, name):
            setattr(db_manager, name, func)


_ensure_db_manager_fallbacks()
